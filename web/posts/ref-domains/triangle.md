...
In the library, these rules are provided by [`TriangleDomain`](LIBROOT/refdomains/triangle.py); its `quadrature(order)` method returns `(points, weights)` for the lowest-order rule that integrates polynomials of the requested degree exactly, up to a maximum of order 5.
