...
As with the triangle, there is no canonical quadrature family for the tetrahedron. One uses tabulated symmetric rules from [Keast (1986)](https://doi.org/10.1016/0045-7825(86)90059-9) and other standard sources, supporting orders 1 through 3 with 1, 4, and 5 quadrature points respectively.
...
In the library, these rules are provided by [`TetrahedronDomain`](LIBROOT/refdomains/tetrahedron.py); its `quadrature(order)` method returns `(points, weights)` for the lowest-order rule that integrates polynomials of the requested degree exactly, up to a maximum of order 3.
