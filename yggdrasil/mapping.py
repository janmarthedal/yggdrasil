...
def compute_mapped_quadrature(
    element: ReferenceElement,
    order: int,
    physical_coords: NDArray[np.float64],
) -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
    """Compute quadrature points, weights, and Jacobian determinants in physical space.

    Parameters
    ----------
    element : ReferenceElement
    order : int — polynomial order for quadrature
    physical_coords : (num_nodes, spatial_dim)

    Returns
    -------
    phys_points : (num_quad, spatial_dim)
        Quadrature points mapped to physical space.
    weights : (num_quad,)
        Quadrature weights (reference domain weights, NOT scaled by det_J).
    det_J : (num_quad,)
        Jacobian determinants at each quadrature point.
    """
    xi, weights = element.domain.quadrature(order)
    J = compute_jacobian(element, xi, physical_coords)
    ...
    if topo_dim == spatial_dim:
        ...
        g = np.einsum("qij,qik->qjk", J, J)
        det_J = np.sqrt(np.abs(np.linalg.det(g)))

    # Map quadrature points to physical space
    N = element.shape_functions(xi)  # (num_quad, num_nodes)
    phys_points = N @ physical_coords  # (num_quad, spatial_dim)

    return phys_points, weights, det_J
