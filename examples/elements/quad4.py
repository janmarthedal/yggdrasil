"""Plot the four shape functions of the Quad4 element as 2D colormaps."""

import numpy as np
import matplotlib.pyplot as plt

from yggdrasil.elements import Quad4
...
for i in range(element.num_nodes):
    ...
    ax.set_xlabel("ξ")
    ax.set_ylabel("η")
    ax.set_title(f"Quad4 — Shape function N{i}")
    ax.set_aspect("equal")
    fig.tight_layout()
    fig.savefig(f"quad4_n{i}.png", dpi=150)
    plt.close(fig)
    print(f"Wrote quad4_n{i}.png")
